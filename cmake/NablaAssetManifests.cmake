...
function(nam_add_channel_target)
    ...
    if (NOT DEFINED NAM_DESTINATION_ROOT OR "${NAM_DESTINATION_ROOT}" STREQUAL "")
        set(NAM_DESTINATION_ROOT "${CMAKE_CURRENT_BINARY_DIR}/media")
    endif()
    ...
    set(_link_root "${CMAKE_CURRENT_SOURCE_DIR}/nam-data/${NAM_TARGET}")
    set(_fetch_script "${_build_root}/NablaAssetManifestsExternalDataFetch.cmake")
    file(MAKE_DIRECTORY "${_build_root}" "${_link_root}")
    ...
    foreach(_asset IN LISTS _items)
        ...
        if (_kind STREQUAL "file")
            set(_data_name "nam-data/${NAM_TARGET}/${_relative_path}")
        elseif(_kind STREQUAL "archive")
            set(_data_name "nam-data/${NAM_TARGET}/nam_archives/${_relative_path}.zip")
        else()
        ...
        list(APPEND _data_refs "DATA{${_data_name}}")
        list(APPEND _entry_kinds "${_kind}")
        list(APPEND _entry_relpaths "${_relative_path}")
    endforeach()

    set(_data_target "${NAM_TARGET}__externaldata")
    set(_old_suppress_dev "${CMAKE_SUPPRESS_DEVELOPER_WARNINGS}")
    set(CMAKE_SUPPRESS_DEVELOPER_WARNINGS 1)
    ExternalData_Expand_Arguments("${_data_target}" _expanded_paths ${_data_refs})
    set(CMAKE_SUPPRESS_DEVELOPER_WARNINGS "${_old_suppress_dev}")
    ExternalData_Add_Target("${_data_target}" SHOW_PROGRESS "${NAM_SHOW_PROGRESS}")

    set(_externaldata_config "${CMAKE_CURRENT_BINARY_DIR}/${_data_target}_config.cmake")
    ...
    add_dependencies("${NAM_TARGET}" "${_data_target}")

    list(LENGTH _expanded_paths _expanded_count)
    math(EXPR _expanded_last "${_expanded_count} - 1")
    set(_archive_count 0)
    foreach(_index RANGE ${_expanded_last})
        list(GET _entry_kinds ${_index} _kind)
        list(GET _entry_relpaths ${_index} _relative_path)
        list(GET _expanded_paths ${_index} _expanded_path)
        set(_target_path "${NAM_DESTINATION_ROOT}/${_relative_path}")
        set(_stamp "${_build_root}/stamps/${_index}.stamp")
        get_filename_component(_stamp_dir "${_stamp}" DIRECTORY)
        file(MAKE_DIRECTORY "${_stamp_dir}")

        if (_kind STREQUAL "archive")
        ...
        add_custom_command(
            OUTPUT "${_stamp}"
            COMMAND "${CMAKE_COMMAND}" -DTYPE=${_type} -DINPUT=${_expanded_path} -DDESTINATION=${_target_path} -DNO_SYMLINKS=$<BOOL:${NAM_NO_SYMLINKS}> -P "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/NablaAssetManifestsMaterialize.cmake"
